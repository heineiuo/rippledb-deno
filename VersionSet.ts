...
import { getCurrentFilename, getManifestFilename } from "./Filename.ts";
import Slice from "./Slice.ts";
import { getExpandedCompactionByteSizeLimit, getMaxBytesForLevel, } from "./VersionFormat.ts";
import Status from "./Status.ts";
import { FileMetaData } from "./VersionFormat.ts";
import VersionBuilder from "./VersionBuilder.ts";
...
export default class VersionSet {
    ...
    constructor(dbpath: string, options: Options, tableCache: TableCache, internalKeyComparator: InternalKeyComparator) {
        this._dbpath = dbpath;
    ...
    public compactRange(level: number, begin: InternalKey, end: InternalKey): Compaction | void {
    ...
    public recover = async (): Promise<RecoverResult> => {
        ...
        const current = await this._options.env.readFile(getCurrentFilename(this._dbpath), "utf8");
        ...
        const reader = new LogReader(await this._options.env.open(getManifestFilename(this._dbpath, manifestNumber), "r"));
    ...
    public async logAndApply(edit: VersionEdit): Promise<Status> {
        ...
        if (!this.manifestWriter) {
            ...
            manifestFilename = getManifestFilename(this._dbpath, this.manifestFileNumber);
            edit.nextFileNumber = this.nextFileNumber;
            this.manifestWriter = new LogWriter(await this._options.env.open(manifestFilename, "a"));
            if (this._options.debug)
        ...
        if (await status.ok()) {
            ...
            if (this._options.debug)
                this._options.log("DEBUG writeSnapshot fail");
        }
        // If we just created a new descriptor file, install it by writing a
        // new CURRENT file that points to it.
        if ((await status.ok()) && manifestFilename.length > 0) {
            status = new Status(this.writeCurrentFile(this._dbpath, this.manifestFileNumber));
        }
    ...
    private async writeCurrentFile(dbpath: string, manifestFileNumber: number): Promise<void> {
    ...
    public pickCompaction(): Compaction | void {
        ...
        if (shouldSizeCompaction) {
            ...
            for (const f of this._current.files[level]) {
                if (!this.compactPointers[level] ||
                    this.compactPointers[level].length === 0 ||
                    this.internalKeyComparator.compare(f.largest, this.compactPointers[level]) > 0) {
                    c.inputs[0].push(f);
                    break;
                }
            }
     ...
     */
    private getRange(inputs: FileMetaData[], smallest: InternalKey, largest: InternalKey): void {
        ...
        for (let i = 0; i < inputs.length; i++) {
            ...
            if (i === 0) {
                ...
                if (this.internalKeyComparator.compare(fileMetaData.smallest, smallest) <
                    0) {
                    smallest.buffer = fileMetaData.smallest.buffer;
                }
     ...
     * REQUIRES: inputs is not empty
     */
    private getRange2(inputs1: FileMetaData[], inputs2: FileMetaData[], smallest: InternalKey, largest: InternalKey): void {
        const all = inputs1.concat(inputs2);
        this.getRange(all, smallest, largest);
    }
    // Finds the largest key in a vector of files. Returns true if files it not
    // empty.
    private findLargestKey(icmp: InternalKeyComparator, files: FileMetaData[], largestKey: InternalKey): boolean {
        if (files.length === 0)
            return false;
    ...
    private addBoundaryInputs(icmp: InternalKeyComparator, levelFiles: FileMetaData[], compactionFiles: FileMetaData[]): void {
    ...
    public findSmallestBoundaryFile(icmp: InternalKeyComparator, levelFiles: FileMetaData[], largestKey: InternalKey): FileMetaData {
    ...
    public setupOtherInputs(c: Compaction): void {
        ...
        if (c.inputs.length > 0) {
            ...
            if (expand0.length > c.inputs[0].length &&
                ...
                if (expand1.length === c.inputs[1].length) {
                    this._options.log(`Expanding@${level} ${c.inputs[0].length}+${c.inputs[1].length}` +
    ...
    public async *makeInputIterator(currentCompaction: Compaction): AsyncIterableIterator<Entry> {
        ...
        const space = currentCompaction.level === 0
