...
import { encodeFixed64 } from "./Coding.ts";
enum SaverState {
    kNotFound,
    kFound,
    kDeleted,
...
class State {
    ...
    static async match(state: unknown, level: number, f: FileMetaData): Promise<boolean> {
        ...
        state.s = await state.vset.tableCache.get(state.options, f.number, f.fileSize, state.ikey, state.saver, Version.saveValue);
        if (!(await state.s.ok())) {
        ...
        switch (state.saver.state) {
            ...
            case SaverState.kCorrupt:
                state.s = Status.createCorruption(`corrupted key for ${state.saver.userKey.toString()}`);
                state.found = true;
                return false;
        }
        return false;
...
export default class Version {
    ...
    static *levelFileNumIterator(icmp: InternalKeyComparator, // TODO used for seek
    files: FileMetaData[]): IterableIterator<FileEntry> {
    ...
    static beforeFile(ucmp: Comparator, userKey: Slice, f: FileMetaData): boolean {
    ...
    public async forEachOverlapping(userKey: Slice, internalKey: Slice, arg: unknown, match: (arg: unknown, level: number, f: FileMetaData) => Promise<boolean>): Promise<void> {
    ...
    private someFileOverlapsRange(icmp: InternalKeyComparator, disjointSortedFile: boolean, files: FileMetaData[], smallestUserKey: Slice, largestUserKey: Slice): boolean {
        const ucmp = icmp.userComparator;
        if (!disjointSortedFile) {
            // Need to check against all files
            for (let i = 0; i < files.length; i++) {
                const f = files[i];
                if (Version.afterFile(ucmp, smallestUserKey, f) ||
        ...
        if (!!smallestUserKey) {
            ...
            const smallkey = new InternalKey(smallestUserKey, InternalKey.kMaxSequenceNumber, kValueTypeForSeek);
    ...
    // binary search
    private findFile(icmp: InternalKeyComparator, files: FileMetaData[], key: Slice): number {
        let left = 0; //  uint32_t
    ...
    public overlapInLevel(level: number, smallestUserKey: Slice, largestUserKey: Slice): boolean {
    ...
    public getOverlappingInputs(level: number, begin: InternalKey, end: InternalKey): FileMetaData[] {
        assert(level >= 0);
    ...
    public pickLevelForMemTableOutput(minUserKey: Slice, maxUserKey: Slice): number {
        let level = 0;
