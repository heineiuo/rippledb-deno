...
import SkiplistNode from "./SkiplistNode.ts";
...
export default class Skiplist {
    ...
    private findGreaterOrEqual(key: Slice, shouldUpdatePrevNodes?: SkiplistNode[]): SkiplistNode {
        ...
        while (true) {
            ...
            if (this.isKeyAfterNode(key, next)) {
                ...
                if (shouldUpdatePrevNodes)
                    shouldUpdatePrevNodes[level] = current;
                if (level === 0) {
                    return next;
                }
                else {
