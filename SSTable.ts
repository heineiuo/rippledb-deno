...
import Footer from "./SSTableFooter.ts";
import DataBlock from "./SSTableBlock.ts";
import FilterBlock from "./SSTableFilterBlock.ts";
import { BlockContents, BlockHandle, CompressionTypes, kBlockTrailerSize, InternalKey, Entry, } from "./Format.ts";
import Status from "./Status.ts";
import { Options, ReadOptions, defaultReadOptions } from "./Options.ts";
import { assert } from "./DBHelper.ts";
import { Buffer } from "./Buffer.ts";
import { encodeFixed64 } from "./Coding.ts";
// Reader
export default class SSTable {
    static async readBlock(file: FileHandle, options: ReadOptions, handle: BlockHandle): Promise<BlockContents> {
        const result = {
            data: new Slice(),
            cachable: false,
            heapAllocated: false,
    ...
    static async open(options: Options, file: FileHandle): Promise<SSTable> {
        ...
        await file.read(footerBuf, 0, footerBuf.length, stat.size - Footer.kEncodedLength);
        const footer = new Footer(footerBuf);
        const indexBlockContents = await this.readBlock(file, defaultReadOptions, footer.indexHandle);
    ...
    private async readMeta(footer: Footer): Promise<void> {
        ...
        const contents = await SSTable.readBlock(this._file, defaultReadOptions, footer.metaIndexHandle);
        ...
        for (const handleValue of this._indexBlock.iterator(this._options.comparator)) {
            ...
            if (!!this._filterReader &&
                ...
                for await (const entry of this.blockIterator(this, this._options, handle, false, "datablock")) {
                    ...
                    if (entryInternalKey.userKey.isEqual(targetInternalKey.userKey) &&
    ...
    async *blockIterator(table: SSTable, options: Options, handle: BlockHandle, reverse = false, blockType?: string): AsyncIterableIterator<Entry> {
        ...
        if (!dataBlock) {
            ...
            const { bytesRead } = await this._file.read(data, 0, data.length, handle.offset);
