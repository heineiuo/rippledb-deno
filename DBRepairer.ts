 ...
 */
import { Options, DatabaseOptions, defaultOptions, ReadOptions, } from "./Options.ts";
import { parseFilename, getLogFilename, getTableFilename, getTempFilename, getManifestFilename, setCurrentFile, } from "./Filename.ts";
import { FileType, InternalKeyComparator, SequenceNumber, parseInternalKey, ParsedInternalKey, } from "./Format.ts";
...
****`);
    ...
    async findFiles(): Promise<void | Error> {
        const filenames = (await this._options.env.readdir(this._dbpath)).reduce((filenames: string[], dirent) => {
            if (dirent.isFile()) {
    ...
    async convertLogToTable(logNumber: number): Promise<void> {
        ...
        const status = await buildTable(this._dbpath, this._env, this._options, mem.iterator(), meta);
        mem.unref();
        if (await status.ok()) {
            if (meta.fileSize > 0) {
    ...
    async repairTable(srcTableFilename: string, tableInfo: TableInfo): Promise<void | Error> {
        // We will copy src contents to a new table and then rename the
        // new table over the source.
        const destTableFilename = getTableFilename(this._dbpath, this._nextFileNumber++);
        let error: Error | void;
        try {
            let counter = 0;
