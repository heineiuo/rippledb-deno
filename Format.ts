...
import { decodeFixed64, encodeFixed64, decodeFixed32 } from "./Coding.ts";
...
export class InternalKeyComparator implements Comparator {
    ...
    findShortestSeparator(start: Slice, limit: Slice): void {
        ...
        if (tmp.size < userStart.size &&
            ...
            tmp.buffer = Buffer.concat([
                tmp.buffer,
                encodeFixed64(packSequenceAndType(kMaxSequenceNumber, kValueTypeForSeek)),
            ]);
    ...
    compare(key1: Slice, key2: Slice): number {
        ...
        if (r !== 0)
            return r;
        const sn1 = decodeFixed64(Buffer.concat([
            key1.buffer.slice(key1.size - 8, key1.size - 1),
            this.oneByte,
        ]));
        const sn2 = decodeFixed64(Buffer.concat([
...
export function parseInternalKey(internalKey: Slice, ikey: ParsedInternalKey): boolean {
    try {
...
export class LookupKey {
    ...
    constructor(userKey: Slice, sequence: SequenceNumber) {
        ...
        this._internalKeySizeBuf = Buffer.fromUnknown(varint.encode(userKey.size + 8));
        this._sequenceBuf = sequence.toFixed64Buffer();
        this._sequenceBuf.fillBuffer(Buffer.fromUnknown(varint.encode(kValueTypeForSeek)), 7);
    }
