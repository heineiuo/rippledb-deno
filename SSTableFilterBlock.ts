...
import { FilterPolicy } from "./Options.ts";
...
export default class SSTableFilterBlock {
    ...
    public keyMayMatch(blockOffset: number, key: Slice): boolean {
        ...
        if (index < this._num) {
            ...
            const limit = decodeFixed32(this._buffer.slice(this._offset + index * 4 + 4));
            if (start <= limit && limit <= this._offset + this._size) {
                const filter = new Slice(this._buffer.slice(this._offset + start, this._offset + limit - start));
