...
import { encodeFixed32 } from "./Coding.ts";
import { Options } from "./Options.ts";
import { BlockHandle, CompressionTypes, kBlockTrailerSize, kSizeOfUInt32, } from "./Format.ts";
export default class SSTableBuilder {
    ...
    async add(key: Slice, value: Slice): Promise<void> {
        ...
        if (this._numberOfEntries > 0) {
            assert(this._options.comparator.compare(key, this._lastKey) > 0, `new key must bigger then last key`);
        }
    ...
    async writeRawBlock(blockContent: Slice, type: CompressionTypes, handle: BlockHandle): Promise<void> {
        ...
        trailer[0] = type;
        const crc32buffer = crc32(Buffer.concat([blockContent.buffer, Buffer.fromUnknown([type])]));
    ...
    async finish(): Promise<void> {
        ...
        if (!!this._metaBlock) {
            await this.writeRawBlock(this._metaBlock.finish(), CompressionTypes.none, filterBlockHandle);
        }
...
class BlockBuilder {
    ...
    public add(key: Slice, value: Slice): void {
        ...
        this._lastKey = new Slice(Buffer.concat([
...
class FilterBlockBuilder {
    ...
    public generateFilter(): void {
        ...
        for (let i = 0; i < numKeys; i++) {
            ...
            this._tempKeys[i] = new Slice(this._keys.slice(this._starts[i], this._starts[i + 1]));
        }
    ...
    public finish(): Slice {
        ...
        this._result = Buffer.concat([
            ...
            Buffer.fromUnknown([FilterBlockBuilder.kFilterBaseLg]),
        ]);
