...
import { InternalKey, VersionEditTag } from "./Format.ts";
...
export default class VersionEditRecord {
    ...
    static add(edit: VersionEdit): Slice {
        ...
        edit.compactPointers.forEach((pointer: {
            ...
            bufList.push(Buffer.fromArrayLike(varint.encode(pointer.internalKey.length)));
        ...
        edit.newFiles.forEach((file: NewFile) => {
            ...
            bufList.push(Buffer.fromArrayLike(varint.encode(file.level)));
            bufList.push(Buffer.fromArrayLike(varint.encode(file.fileMetaData.number)));
            bufList.push(Buffer.fromArrayLike(varint.encode(file.fileMetaData.fileSize)));
            bufList.push(Buffer.fromArrayLike(varint.encode(file.fileMetaData.smallest.length)));
            bufList.push(file.fileMetaData.smallest.buffer);
            bufList.push(Buffer.fromArrayLike(varint.encode(file.fileMetaData.largest.length)));
    ...
    static decode(opSlice: Slice): VersionEdit {
        ...
        while (index < opSlice.length) {
            ...
            if (type === VersionEditTag.kComparator) {
                ...
                const internalKey = new Slice(opBuffer.slice(index, index + internalKeyLength));
    ...
    type: VersionEditTag;
    get buffer(): Buffer {
        const lengthBuf = Buffer.fromHex(createHexStringFromDecimal(this.data.length));
