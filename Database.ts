...
import { defaultOptions, defaultWriteOptions, Options, DatabaseOptions, ReadOptions, WriteOptions, IteratorOptions, defaultIteratorOptions, } from "./Options.ts";
import IteratorHelper from "./IteratorHelper.ts";
import { ValueType, Config, FileType, InternalKeyComparator, parseInternalKey, ParsedInternalKey, InternalKey, SequenceNumber, kValueTypeForSeek, LookupKey, Entry, } from "./Format.ts";
import { FileMetaData, GetStats } from "./VersionFormat.ts";
import Version from "./Version.ts";
import Compaction, { CompactionState, CompactionStateOutput, CompactionStats, } from "./Compaction.ts";
import Slice from "./Slice.ts";
import VersionSet from "./VersionSet.ts";
import VersionEdit from "./VersionEdit.ts";
import VersionEditRecord from "./VersionEditRecord.ts";
import { parseFilename, getCurrentFilename, getLogFilename, getManifestFilename, getTableFilename, getOldInfoLogFilename, getInfoLogFilename, getLockFilename, } from "./Filename.ts";
import { WriteBatch, WriteBatchInternal } from "./WriteBatch.ts";
import Status from "./Status.ts";
import SSTableBuilder from "./SSTableBuilder.ts";
import { BytewiseComparator, Comparator } from "./Comparator.ts";
import { FileHandle, Dirent } from "./Env.ts";
import { TableCache, TableAndFile } from "./SSTableCache.ts";
import { Snapshot, SnapshotList } from "./Snapshot.ts";
...
export default class Database {
    ...
    private async initVersionEdit(): Promise<void> {
        ...
        await writer.close();
        await this._options.env.writeFile(getCurrentFilename(this._dbpath), "MANIFEST-000001\n");
    }
    private async recoverWrapper(): Promise<void> {
        try {
            ...
            if ((await status.ok()) && !this._memtable) {
                ...
                this._log = new LogWriter(await this._options.env.open(getLogFilename(this._dbpath, newLogNumber), "a"));
                this._memtable = new MemTable(this._internalKeyComparator);
                this._memtable.ref();
            }
    ...
    private async recover(): Promise<RecoverResult> {
        ...
        if (await this.shouldInit()) {
            ...
            try {
                await this._options.env.rename(getInfoLogFilename(this._dbpath), getOldInfoLogFilename(this._dbpath));
            }
            catch (e) { }
        }
        this._options.infoLog = await this._options.env.open(getInfoLogFilename(this._dbpath), "a+");
        ...
        const filenames = (await this._options.env.readdir(this._dbpath)).reduce((filenames: string[], dirent: Dirent) => {
            if (dirent.isFile()) {
        ...
        for (const filename of filenames) {
            ...
            if (internalFile.isInternalFile) {
                ...
                if (internalFile.type == FileType.kLogFile &&
    ...
    public async *iterator(options: IteratorOptions = {}): AsyncIterableIterator<{
        key: Uint8Array;
        value: Uint8Array;
    }> {
        if (!this._ok)
        ...
        this._memtable.ref();
        iteratorList.push(IteratorHelper.wrap(IteratorHelper.makeAsync(this._memtable.iterator(reverse)), () => {
            this._memtable.unref();
        }));
        if (this._immtable) {
            this._immtable.ref();
            iteratorList.push(IteratorHelper.wrap(IteratorHelper.makeAsync(this._immtable.iterator(reverse)), () => {
            ...
            if (files.length > 0) {
                for (const file of files) {
                    ...
                    if (await status.ok()) {
                        ...
                        iteratorList.push(IteratorHelper.wrap(tf.table.entryIterator(reverse), async () => {
                            file.refs--;
                            current.unref();
                            await tf.file.close();
                        }));
        ...
        const merger = new IteratorMerger(this._internalKeyComparator, iteratorList, iteratorList.length);
     ...
     * table.get
     */
    public async get(userKey: string | Uint8Array, options: ReadOptions = {}): Promise<Uint8Array | void> {
        if (!this._ok)
            await this.ok();
     ...
     */
    public put(key: string | Uint8Array, value: string | Uint8Array, options?: WriteOptions): Promise<void> {
        const batch = new WriteBatch();
        batch.put(key, value);
    ...
    private buildBatchGroup(): [Writer, WriteBatch] {
        ...
        if (!first)
            throw new Error(`writer is empty`);
        let result = first.batch;
        if (!result)
            throw new Error(`first batch is empty`);
    ...
    private async makeRoomForWrite(force: boolean): Promise<Status> {
        ...
        while (true) {
            ...
            else if (this._versionSet.getNumLevelFiles(0) >= Config.kL0StopWritesTrigger) {
    ...
    private async maybeScheduleCompaction(): Promise<void> {
        if (this._options.debug)
            this._options.log(`DEBUG !this._immtable=${!this
    ...
    private async backgroundCompaction(): Promise<void> {
        ...
        if (!!this._manualCompaction) {
            ...
            // Manual compaction ...
            if (this._options.debug)
                this._options.log("DEBUG Manual compaction ...");
        }
        else {
    ...
    private async writeLevel0Table(mem: MemTable, edit: VersionEdit, base?: Version): Promise<Status> {
        if (mem.size === 0)
            return new Status(Promise.resolve());
    ...
    private async openCompactionOutputFile(compact: CompactionState): Promise<Status> {
        assert(!!compact);
        assert(!compact.builder);
        ...
        if (await status.ok()) {
            ...
            if (this._options.debug)
                this._options.log("DEBUG open file success");
        }
        else {
            if (this._options.debug)
    ...
    private async installCompactionResults(compact: CompactionState): Promise<Status> {
        this._options.log(`Compacted ${compact.compaction.numInputFiles(0)}@${compact.compaction.level} + ${compact.compaction.numInputFiles(1)}@${compact.compaction.level + 1} files => ${compact.totalBytes} bytes"`);
        // Add compaction outputs
        compact.compaction.addInputDeletions(compact.compaction.edit);
        const level = compact.compaction.level;
        if (this._options.debug)
            this._options.log(`DEBUG compact.outputs.length=${compact.outputs.length}`);
        for (let i = 0; i < compact.outputs.length; i++) {
            const out = compact.outputs[i];
            compact.compaction.edit.addFile(level + 1, out.number, out.fileSize, out.smallest, out.largest);
        }
        if (this._options.debug)
            this._options.log(`DEBUG installCompactionResults logAndApply starting...`);
     ...
     * manually compact
     */
    public async compactRange(begin: Uint8Array | string, end: Uint8Array | string): Promise<void> {
        if (!this._ok)
            await this.ok();
    ...
    private async manualCompactRangeWithLevel(level: number, begin: Slice, end: Slice): Promise<void> {
        if (this._options.debug)
        ...
        if (!begin) {
            ...
            beginStorage = new InternalKey(begin, InternalKey.kMaxSequenceNumber, kValueTypeForSeek);
        ...
        while (!manual.done) {
            if (!this._manualCompaction) {
                ...
                if (this._options.debug)
                    this._options.log("DEBUG Running either my compaction or another compaction.");
    ...
    private async finishCompactionOutputFile(compact: CompactionState, inputStatus: Status): Promise<Status> {
        let status = new Status();
        assert(!!compact);
        assert(!!compact.outfile);
        assert(!!compact.builder);
        ...
        delete compact.outfile;
        if (currentEntries > 0) {
            status = new Status(this._options.env.access(getTableFilename(this._dbpath, outputNumber)));
            if (await status.ok()) {
